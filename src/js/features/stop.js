...
(function ($) {
	...
	$.extend(MediaElementPlayer.prototype, {
		...
		buildstop: function (player, controls, layers, media) {
			...
			$('<div class="' +  t.options.classPrefix + 'button ' + t.options.classPrefix + 'stop-button ' +
				t.options.classPrefix + 'stop">' +
				'<button type="button" aria-controls="' + t.id + '" ' +
					'title="' + stopTitle + '" aria-label="' + stopTitle + '"></button>' +
			'</div>')
			.appendTo(controls)
			.click(function () {
				if (!media.paused) {
				...
				if (media.currentTime > 0) {
					...
					.html(mejs.Utils.secondsToTimeCode(0, player.options.alwaysShowHours));
