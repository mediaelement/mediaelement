...
FlashMediaElementRenderer = {
	create: function (mediaElement, options, mediaFiles) {
		...
		win['__ready__' + flash.id] = function() {
			...
			//console.log('' + options.prefix + ' ready', flash); // , flash.flashApi);
	
	
			//console.log('FLASH, running stack');
			// do call stack
		...
		win['__event__' + flash.id] = function(eventName, values) {
			...
			/*
			if (eventName.indexOf('click') > -1 || eventName.indexOf('mouse') > -1) {
				console.log(flash.id, 'mouse event', eventName, values);
			}
			
			if (eventName.indexOf('play') > -1) {
				console.log(flash.id, 'play event', eventName, values);
			}
			*/
