...
! function (context) {
	var __uid = 1,
		...
		removeListener = function (element, orgType, handler) {
			var uid, names, uids, i, events = retrieveEvents(element),
...
! function (context, win) {
	...
	bonzo.create = function (node) {
		return typeof node == 'string' ?
			function () {
				var tag = /^<([^\s>]+)/.exec(node);
				var el = doc.createElement(tag && tagMap[tag[1].toLowerCase()] || 'div'),
					els = [];
				el.innerHTML = node;
	...
	function dimension(v, self, which) {
		return v ?
			...
			function (r) {
				...
				return isNaN(r) ? self[0]['offset' + which.replace(/^\w/, function (m) {
...
! function (context, doc) {
	...
	function _qwery(selector) {
		var r = [],
			ret = [],
			i, j = 0,
			k, l, m, p, token, tag, els, root, intr, item, children,
...
! function (doc) {
	...
	var table = 'table',
		...
		function create(node, root) {
			...
			var el = (root || doc).createElement(nodeMap[tag] || 'div'),
				els = [];
			el.innerHTML = node;
			var nodes = el.childNodes;
			el = el.firstChild;
