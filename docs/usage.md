...
With `MediaElement`, the way to do it is just setting the new values for required attributes (`srclang`, `kind` and `src`) and optionally, `label`, 
...
After that, it is required to register the new track(s) as follows:
```javascript
player.findTracks();
