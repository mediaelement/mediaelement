...
h=f('<div class="mejs-overlay mejs-layer"><div class="mejs-overlay-error"></div></div>').hide().appendTo(b),i=f('<div class="mejs-overlay mejs-layer mejs-overlay-play"><div class="mejs-overlay-button"></div></div>').appendTo(b).click(function(){d.paused?d.play():d.pause()});if(mejs.MediaFeatures.isiOS||mejs.MediaFeatures.isAndroid){i.remove();e.remove()}d.addEventListener("play",function(){i.hide();h.hide()},false);d.addEventListener("pause",function(){i.show()},false);d.addEventListener("loadstart",
...
i=c.find(".mejs-time-handle"),j=c.find(".mejs-time-float"),k=c.find(".mejs-time-float-current"),n=function(g){g=g.pageX;var m=e.offset(),q=e.outerWidth(),o=0;o=0;if(g>m.left&&g<=q+m.left&&d.duration){o=(g-m.left)/q;o=o<=0.02?0:o*d.duration;l&&d.setCurrentTime(o);j.css("left",g-m.left);k.html(mejs.Utility.secondsToTimeCode(o))}},l=false,r=false;e.bind("mousedown",function(g){if(g.which===1){l=true;n(g);return false}});c.find(".mejs-time-rail").bind("mouseenter",function(){r=true}).bind("mouseleave",
function(){r=false});f(document).bind("mouseup",function(){l=false}).bind("mousemove",function(g){if(l||r)n(g)});d.addEventListener("progress",function(g){a.setProgressRail(g);a.setCurrentRail(g)},false);d.addEventListener("timeupdate",function(g){a.setProgressRail(g);a.setCurrentRail(g)},false);this.loaded=b;this.total=e;this.current=h;this.handle=i},setProgressRail:function(a){var c=a!=undefined?a.target:this.media,b=null;if(c&&c.buffered&&c.buffered.length>0&&c.buffered.end&&c.duration)b=c.buffered.end(0)/
...
(function(f){f.extend(MediaElementPlayer.prototype,{buildvolume:function(a,c,b,d){var e=f('<div class="mejs-button mejs-volume-button mejs-mute"><button type="button"></button><div class="mejs-volume-slider"><div class="mejs-volume-total"></div><div class="mejs-volume-current"></div><div class="mejs-volume-handle"></div></div></div>').appendTo(c),h=e.find(".mejs-volume-slider"),i=e.find(".mejs-volume-total"),j=e.find(".mejs-volume-current"),k=e.find(".mejs-volume-handle"),n=function(g){if(h.is(":visible")){var m=
i.height(),q=i.position();g=m-m*g;k.css("top",q.top+g-k.height()/2);j.height(m-g);j.css("top",q.top+g)}else{h.show();n(g);h.hide()}},l=function(g){var m=i.height(),q=i.offset(),o=parseInt(i.css("top").replace(/px/,""),10);g=g.pageY-q.top;var p=(m-g)/m;if(q.top!=0){p=Math.max(0,p);p=Math.min(p,1);if(g<0)g=0;else if(g>m)g=m;k.css("top",g-k.height()/2+o);j.height(m-g);j.css("top",g+o);if(p==0){d.setMuted(true);e.removeClass("mejs-mute").addClass("mejs-unmute")}else{d.setMuted(false);e.removeClass("mejs-unmute").addClass("mejs-mute")}p=
...
/\r?\n/);for(var b={text:[],times:[]},d,e;c<a.length;c++)if(this.pattern_identifier.exec(a[c])){c++;if((d=this.pattern_timecode.exec(a[c]))&&c<a.length){c++;e=a[c];for(c++;a[c]!==""&&c<a.length;){e=e+"\n"+a[c];c++}b.text.push(e);b.times.push({start:mejs.Utility.timeCodeToSeconds(d[1]),stop:mejs.Utility.timeCodeToSeconds(d[3]),settings:d[5]})}}return b},translateTrackText:function(a,c,b,d,e){var h={text:[],times:[]},i,j;this.translateText(a.text.join(" <a></a>"),c,b,d,function(k){i=k.split("<a></a>");
for(j=0;j<a.text.length;j++){h.text[j]=i[j];h.times[j]={start:a.times[j].start,stop:a.times[j].stop,settings:a.times[j].settings}}e(h)})},translateText:function(a,c,b,d,e){for(var h,i=[],j,k="",n=function(){if(i.length>0){j=i.shift();mejs.TrackFormatParser.translateChunk(j,c,b,d,function(l){if(l!="undefined")k+=l;n()})}else e(k)};a.length>0;)if(a.length>1E3){h=a.lastIndexOf(".",1E3);i.push(a.substring(0,h));a=a.substring(h+1)}else{i.push(a);a=""}n()},translateChunk:function(a,c,b,d,e){a={q:a,langpair:c+
...
var a=this;a.killContextMenuTimer();a.contextMenuTimer=setTimeout(function(){a.hideContextMenu();a.killContextMenuTimer()},750)},killContextMenuTimer:function(){var a=this.contextMenuTimer;console.log("killContextMenuTimer",a);if(a!=null){clearTimeout(a);delete a}},hideContextMenu:function(){this.contextMenu.hide()},renderContextMenu:function(a,c){for(var b=this,d="",e=b.options.contextMenuItems,h=0,i=e.length;h<i;h++)if(e[h].isSeparator)d+='<div class="mejs-contextmenu-separator"></div>';else{var j=
e[h].render(b);if(j!=null)d+='<div class="mejs-contextmenu-item" data-itemindex="'+h+'" id="element-'+Math.random()*1E6+'">'+j+"</div>"}b.contextMenu.empty().append(f(d)).css({top:c,left:a}).show();b.contextMenu.find(".mejs-contextmenu-item").each(function(){var k=f(this),n=parseInt(k.data("itemindex"),10),l=b.options.contextMenuItems[n];typeof l.show!="undefined"&&l.show(k,b);k.click(function(){typeof l.click!="undefined"&&l.click(b);b.contextMenu.hide()})});setTimeout(function(){b.killControlsTimer("rev3")},
100)}})})(mejs.$);
