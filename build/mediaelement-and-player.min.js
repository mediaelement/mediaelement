...
"volumeVertical","fullscreen"],isVideo:true,enableKeyboard:true,pauseOtherPlayers:true,keyActions:[{keys:[32,179],action:function(a,c){c.paused||c.ended?c.play():c.pause()}},{keys:[38],action:function(a,c){c.setVolume(Math.min(c.volume+0.1,1))}},{keys:[40],action:function(a,c){c.setVolume(Math.max(c.volume-0.1,0))}},{keys:[37,227],action:function(a,c){if(!isNaN(c.duration)&&c.duration>0){if(a.isVideo){a.showControls();a.startControlsTimer()}c.setCurrentTime(Math.min(c.currentTime-c.duration*0.05,
c.duration))}}},{keys:[39,228],action:function(a,c){if(!isNaN(c.duration)&&c.duration>0){if(a.isVideo){a.showControls();a.startControlsTimer()}c.setCurrentTime(Math.max(c.currentTime+c.duration*0.05,0))}}},{keys:[70],action:function(a){if(typeof a.enterFullScreen!="undefined")a.isFullScreen?a.exitFullScreen():a.enterFullScreen()}}]};mejs.mepIndex=0;mejs.players=[];mejs.MediaElementPlayer=function(a,c){if(!(this instanceof mejs.MediaElementPlayer))return new mejs.MediaElementPlayer(a,c);this.$media=
this.$node=f(a);this.node=this.media=this.$media[0];if(typeof this.node.player!="undefined")return this.node.player;else this.node.player=this;if(typeof c=="undefined")c=this.$node.data("mejsoptions");this.options=f.extend({},mejs.MepDefaults,c);mejs.players.push(this);this.init();return this};mejs.MediaElementPlayer.prototype={hasFocus:false,controlsAreVisible:true,init:function(){var a=this,c=mejs.MediaFeatures,b=f.extend(true,{},a.options,{success:function(e,h){a.meReady(e,h)},error:function(e){a.handleError(e)}}),
...
!(d.isiPhone&&b.options.iPhoneUseNativeControls)){b.buildposter(b,b.controls,b.layers,b.media);b.buildkeyboard(b,b.controls,b.layers,b.media);b.buildoverlays(b,b.controls,b.layers,b.media);b.findTracks();for(h in b.options.features){d=b.options.features[h];if(b["build"+d])try{b["build"+d](b,b.controls,b.layers,b.media)}catch(k){}}b.container.trigger("controlsready");b.setPlayerSize(b.width,b.height);b.setControlsSize();if(b.isVideo){if(mejs.MediaFeatures.hasTouch)b.$media.bind("touchstart",function(){if(b.controlsAreVisible)b.hideControls(false);
...
function(){b.controlsEnabled&&!b.media.paused&&!b.options.alwaysShowControls&&b.startControlsTimer(1E3)})}e&&!b.options.alwaysShowControls&&b.hideControls();b.options.enableAutosize&&b.media.addEventListener("loadedmetadata",function(j){if(b.options.videoHeight<=0&&b.domNode.getAttribute("height")===null&&!isNaN(j.target.videoHeight)){b.setPlayerSize(j.target.videoWidth,j.target.videoHeight);b.setControlsSize();b.media.setVideoSize(j.target.videoWidth,j.target.videoHeight)}},false)}a.addEventListener("play",
function(){for(var j=0,n=mejs.players.length;j<n;j++){var m=mejs.players[j];m.id!=b.id&&b.options.pauseOtherPlayers&&!m.paused&&!m.ended&&m.pause();m.hasFocus=false}b.hasFocus=true},false);b.media.addEventListener("ended",function(){try{b.media.setCurrentTime(0)}catch(j){}b.media.pause();b.setProgressRail&&b.setProgressRail();b.setCurrentRail&&b.setCurrentRail();if(b.options.loop)b.media.play();else!b.options.alwaysShowControls&&b.controlsEnabled&&b.showControls()},false);b.media.addEventListener("loadedmetadata",
...
b=c.find("img");if(b.length==0)b=f('<img width="100%" height="100%" />').appendTo(c);b.attr("src",a)},buildoverlays:function(a,c,b,d){if(a.isVideo){var e=f('<div class="mejs-overlay mejs-layer"><div class="mejs-overlay-loading"><span></span></div></div>').hide().appendTo(b),h=f('<div class="mejs-overlay mejs-layer"><div class="mejs-overlay-error"></div></div>').hide().appendTo(b),k=f('<div class="mejs-overlay mejs-layer mejs-overlay-play"><div class="mejs-overlay-button"></div></div>').appendTo(b).click(function(){d.paused?
d.play():d.pause()});d.addEventListener("play",function(){k.hide();e.hide();h.hide()},false);d.addEventListener("playing",function(){k.hide();e.hide();h.hide()},false);d.addEventListener("pause",function(){mejs.MediaFeatures.isiPhone||k.show()},false);d.addEventListener("waiting",function(){e.show()},false);d.addEventListener("loadeddata",function(){e.show()},false);d.addEventListener("canplay",function(){e.hide()},false);d.addEventListener("error",function(){e.hide();h.show();h.find("mejs-overlay-error").html("Error loading this resource")},
false)}},buildkeyboard:function(a,c,b,d){f(document).keydown(function(e){if(a.hasFocus&&a.options.enableKeyboard)for(var h=0,k=a.options.keyActions.length;h<k;h++)for(var j=a.options.keyActions[h],n=0,m=j.keys.length;n<m;n++)if(e.keyCode==j.keys[n]){e.preventDefault();j.action(a,d);return false}return true});f(document).click(function(e){if(f(e.target).closest(".mejs-container").length==0)a.hasFocus=false})},findTracks:function(){var a=this,c=a.$media.find("track");a.tracks=[];c.each(function(b,d){d=
...
k=c.find(".mejs-time-handle"),j=c.find(".mejs-time-float"),n=c.find(".mejs-time-float-current"),m=function(i){i=i.pageX;var g=e.offset(),p=e.outerWidth(),q=0;q=0;var s=i-g.left;if(i>g.left&&i<=p+g.left&&d.duration){q=(i-g.left)/p;q=q<=0.02?0:q*d.duration;o&&d.setCurrentTime(q);if(!mejs.MediaFeatures.hasTouch){j.css("left",s);n.html(mejs.Utility.secondsToTimeCode(q));j.show()}}},o=false,l=false;e.bind("mousedown",function(i){if(i.which===1){o=true;m(i);return false}});c.find(".mejs-time-total").bind("mouseenter",
function(){l=true;mejs.MediaFeatures.hasTouch||j.show()}).bind("mouseleave",function(){l=false;j.hide()});f(document).bind("mouseup",function(){o=false;j.hide()}).bind("mousemove",function(i){if(o||l)m(i)});d.addEventListener("progress",function(i){a.setProgressRail(i);a.setCurrentRail(i)},false);d.addEventListener("timeupdate",function(i){a.setProgressRail(i);a.setCurrentRail(i)},false);this.loaded=b;this.total=e;this.current=h;this.handle=k},setProgressRail:function(a){var c=a!=undefined?a.target:
...
(function(f){f.extend(mejs.MepDefaults,{muteText:"Mute Toggle",hideVolumeOnTouchDevices:true});f.extend(MediaElementPlayer.prototype,{buildvolumeVertical:function(a,c,b,d){if(!(mejs.MediaFeatures.hasTouch&&this.options.hideVolumeOnTouchDevices)){var e=f('<div class="mejs-button mejs-volume-button mejs-mute"><button type="button" aria-controls="'+this.id+'" title="'+this.options.muteText+'"></button><div class="mejs-volume-slider"><div class="mejs-volume-total"></div><div class="mejs-volume-current"></div><div class="mejs-volume-handle"></div></div></div>').appendTo(c),
h=e.find(".mejs-volume-slider"),k=e.find(".mejs-volume-total"),j=e.find(".mejs-volume-current"),n=e.find(".mejs-volume-handle"),m=function(g){if(h.is(":visible")){var p=k.height(),q=k.position();g=p-p*g;n.css("top",q.top+g-n.height()/2);j.height(p-g);j.css("top",q.top+g)}else{h.show();m(g);h.hide()}},o=function(g){var p=k.height(),q=k.offset(),s=parseInt(k.css("top").replace(/px/,""),10);g=g.pageY-q.top;var r=(p-g)/p;if(!(q.top==0||q.left==0)){r=Math.max(0,r);r=Math.min(r,1);if(g<0)g=0;else if(g>
p)g=p;n.css("top",g-n.height()/2+s);j.height(p-g);j.css("top",g+s);if(r==0){d.setMuted(true);e.removeClass("mejs-mute").addClass("mejs-unmute")}else{d.setMuted(false);e.removeClass("mejs-unmute").addClass("mejs-mute")}r=Math.max(0,r);r=Math.min(r,1);d.setVolume(r)}},l=false,i=false;e.hover(function(){h.show();i=true},function(){i=false;l||h.hide()});h.bind("mouseover",function(){i=true}).bind("mousedown",function(g){o(g);l=true;return false});f(document).bind("mouseup",function(){l=false;i||h.hide()}).bind("mousemove",
function(g){l&&o(g)});e.find("button").click(function(){d.setMuted(!d.muted)});d.addEventListener("volumechange",function(){if(!l)if(d.muted){m(0);e.removeClass("mejs-mute").addClass("mejs-unmute")}else{m(d.volume);e.removeClass("mejs-unmute").addClass("mejs-mute")}},false);if(this.container.is(":visible")){m(a.options.startVolume);d.pluginType==="native"&&d.setVolume(a.options.startVolume)}}}})})(mejs.$);
(function(f){f.extend(MediaElementPlayer.prototype,{buildvolumeHorizontal:function(a,c,b,d){function e(l,i){var g=m.width();g=parseInt(g/2);var p=j.width();i=i||j.width()*l;i=Math.max(i,g);i=Math.min(i,p-g);m.css("left",i-g);n.width(i+g)}function h(l){var i=m.width(),g=parseInt(i/2);l=l.pageX-j.offset().left;i=(l-g)/(j.width()-i);if(i<=0){d.setMuted(true);k.removeClass("mejs-mute").addClass("mejs-unmute")}else{d.setMuted(false);k.removeClass("mejs-unmute").addClass("mejs-mute")}i=Math.max(i,0);i=
Math.min(i,1);d.setVolume(i);e(i,l)}var k=f('<div class="mejs-button mejs-volume-button volume-horizontal mejs-mute"><button type="button"></button><div class="mejs-volume-slider volume-horizontal"><div class="mejs-volume-total"></div><div class="mejs-volume-current"></div><div class="mejs-volume-handle"></div></div></div>');b=k.find(".mejs-volume-slider");var j=k.find(".mejs-volume-total"),n=k.find(".mejs-volume-current"),m=k.find(".mejs-volume-handle"),o=false;k.appendTo(c);b.bind("mousedown",function(l){h(l);
o=true;return false});f(document).bind("mousemove",function(l){o&&h(l)});f(document).bind("mouseup",function(){o=false});k.find("button").click(function(){d.setMuted(!d.muted)});d.addEventListener("volumechange",function(l){if(!o)if(d.muted){e(0);k.removeClass("mejs-mute").addClass("mejs-unmute")}else{e(l.target.volume);k.removeClass("mejs-unmute").addClass("mejs-mute")}},true);e(a.options.startVolume);d.pluginType==="native"&&d.setVolume(a.options.startVolume)}})})(mejs.$);
...
true;a.setControlsSize()}else{a.isNativeFullScreen=false;a.exitFullScreen()}});var e=this,h=f('<div class="mejs-button mejs-fullscreen-button"><button type="button" aria-controls="'+e.id+'" title="'+e.options.fullscreenText+'"></button></div>').appendTo(c);if(e.media.pluginType==="native"||!e.options.usePluginFullScreen&&!mejs.MediaFeatures.isFirefox)h.click(function(){mejs.MediaFeatures.hasTrueNativeFullScreen&&mejs.MediaFeatures.isFullScreen()||a.isFullScreen?a.exitFullScreen():a.enterFullScreen()});
else{var k=null;if(document.documentElement.style.pointerEvents===""&&!mejs.MediaFeatures.isOpera){var j=false,n=function(){if(j){m.hide();o.hide();l.hide();h.css("pointer-events","");e.controls.css("pointer-events","");j=false}},m=f('<div class="mejs-fullscreen-hover" />').appendTo(e.container).mouseover(n),o=f('<div class="mejs-fullscreen-hover"  />').appendTo(e.container).mouseover(n),l=f('<div class="mejs-fullscreen-hover"  />').appendTo(e.container).mouseover(n),i=function(){var g={position:"absolute",
top:0,left:0};m.css(g);o.css(g);l.css(g);m.width(e.container.width()).height(e.container.height()-e.controls.height());g=h.offset().left-e.container.offset().left;fullScreenBtnWidth=h.outerWidth(true);o.width(g).height(e.controls.height()).css({top:e.container.height()-e.controls.height()});l.width(e.container.width()-g-fullScreenBtnWidth).height(e.controls.height()).css({top:e.container.height()-e.controls.height(),left:g+fullScreenBtnWidth})};f(document).resize(function(){i()});h.mouseover(function(){if(!e.isFullScreen){var g=
h.offset(),p=a.container.offset();d.positionFullscreenButton(g.left-p.left,g.top-p.top,false);h.css("pointer-events","none");e.controls.css("pointer-events","none");m.show();l.show();o.show();i();j=true}});d.addEventListener("fullscreenchange",function(){n()})}else h.mouseover(function(){if(k!==null){clearTimeout(k);delete k}var g=h.offset(),p=a.container.offset();d.positionFullscreenButton(g.left-p.left,g.top-p.top,true)}).mouseout(function(){if(k!==null){clearTimeout(k);delete k}k=setTimeout(function(){d.hideFullscreenButton()},
1500)})}a.fullscreenBtn=h;f(document).bind("keydown",function(g){if((mejs.MediaFeatures.hasTrueNativeFullScreen&&mejs.MediaFeatures.isFullScreen()||e.isFullScreen)&&g.keyCode==27)a.exitFullScreen()})}},enterFullScreen:function(){var a=this;if(!(a.media.pluginType!=="native"&&(mejs.MediaFeatures.isFirefox||a.options.usePluginFullScreen))){docStyleOverflow=document.documentElement.style.overflow;document.documentElement.style.overflow="hidden";normalHeight=a.container.height();normalWidth=a.container.width();
...
true},disableContextMenu:function(){this.isContextMenuEnabled=false},contextMenuTimeout:null,startContextMenuTimer:function(){var a=this;a.killContextMenuTimer();a.contextMenuTimer=setTimeout(function(){a.hideContextMenu();a.killContextMenuTimer()},750)},killContextMenuTimer:function(){var a=this.contextMenuTimer;if(a!=null){clearTimeout(a);delete a}},hideContextMenu:function(){this.contextMenu.hide()},renderContextMenu:function(a,c){for(var b=this,d="",e=b.options.contextMenuItems,h=0,k=e.length;h<
k;h++)if(e[h].isSeparator)d+='<div class="mejs-contextmenu-separator"></div>';else{var j=e[h].render(b);if(j!=null)d+='<div class="mejs-contextmenu-item" data-itemindex="'+h+'" id="element-'+Math.random()*1E6+'">'+j+"</div>"}b.contextMenu.empty().append(f(d)).css({top:c,left:a}).show();b.contextMenu.find(".mejs-contextmenu-item").each(function(){var n=f(this),m=parseInt(n.data("itemindex"),10),o=b.options.contextMenuItems[m];typeof o.show!="undefined"&&o.show(n,b);n.click(function(){typeof o.click!=
