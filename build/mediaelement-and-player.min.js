...
!b.media.paused&&!b.options.alwaysShowControls&&b.startControlsTimer(1E3)})}e&&!b.options.alwaysShowControls&&b.hideControls();b.options.enableAutosize&&b.media.addEventListener("loadedmetadata",function(i){if(b.options.videoHeight<=0&&b.domNode.getAttribute("height")===null&&!isNaN(i.target.videoHeight)){b.setPlayerSize(i.target.videoWidth,i.target.videoHeight);b.setControlsSize();b.media.setVideoSize(i.target.videoWidth,i.target.videoHeight)}},false)}a.addEventListener("play",function(){for(var i=
0,k=mejs.players.length;i<k;i++){var l=mejs.players[i];l.id!=b.id&&b.options.pauseOtherPlayers&&!l.paused&&!l.ended&&l.pause();l.hasFocus=false}b.hasFocus=true},false);b.media.addEventListener("ended",function(){try{b.media.setCurrentTime(0)}catch(i){}b.media.pause();b.setProgressRail&&b.setProgressRail();b.setCurrentRail&&b.setCurrentRail();if(b.options.loop)b.media.play();else!b.options.alwaysShowControls&&b.controlsEnabled&&b.showControls()},false);b.media.addEventListener("loadedmetadata",function(){b.updateDuration&&
...
c,b,d){f(document).keydown(function(e){if(a.hasFocus&&a.options.enableKeyboard)for(var g=0,j=a.options.keyActions.length;g<j;g++)for(var i=a.options.keyActions[g],k=0,l=i.keys.length;k<l;k++)if(e.keyCode==i.keys[k]){e.preventDefault();i.action(a,d);return false}return true});f(document).click(function(e){if(f(e.target).closest(".mejs-container").length==0)a.hasFocus=false})},findTracks:function(){var a=this,c=a.$media.find("track");a.tracks=[];c.each(function(){a.tracks.push({srclang:f(this).attr("srclang").toLowerCase(),
...
j=c.find(".mejs-time-handle"),i=c.find(".mejs-time-float"),k=c.find(".mejs-time-float-current"),l=function(h){h=h.pageX;var n=e.offset(),q=e.outerWidth(),o=0;o=0;if(h>n.left&&h<=q+n.left&&d.duration){o=(h-n.left)/q;o=o<=0.02?0:o*d.duration;m&&d.setCurrentTime(o);i.css("left",h-n.left);k.html(mejs.Utility.secondsToTimeCode(o))}},m=false,r=false;e.bind("mousedown",function(h){if(h.which===1){m=true;l(h);return false}});c.find(".mejs-time-total").bind("mouseenter",function(){r=true}).bind("mouseleave",
function(){r=false});f(document).bind("mouseup",function(){m=false}).bind("mousemove",function(h){if(m||r)l(h)});d.addEventListener("progress",function(h){a.setProgressRail(h);a.setCurrentRail(h)},false);d.addEventListener("timeupdate",function(h){a.setProgressRail(h);a.setCurrentRail(h)},false);this.loaded=b;this.total=e;this.current=g;this.handle=j},setProgressRail:function(a){var c=a!=undefined?a.target:this.media,b=null;if(c&&c.buffered&&c.buffered.length>0&&c.buffered.end&&c.duration)b=c.buffered.end(0)/
...
g=e.find(".mejs-volume-slider"),j=e.find(".mejs-volume-total"),i=e.find(".mejs-volume-current"),k=e.find(".mejs-volume-handle"),l=function(h){if(g.is(":visible")){var n=j.height(),q=j.position();h=n-n*h;k.css("top",q.top+h-k.height()/2);i.height(n-h);i.css("top",q.top+h)}else{g.show();l(h);g.hide()}},m=function(h){var n=j.height(),q=j.offset(),o=parseInt(j.css("top").replace(/px/,""),10);h=h.pageY-q.top;var p=(n-h)/n;if(q.top!=0){p=Math.max(0,p);p=Math.min(p,1);if(h<0)h=0;else if(h>n)h=n;k.css("top",
h-k.height()/2+o);i.height(n-h);i.css("top",h+o);if(p==0){d.setMuted(true);e.removeClass("mejs-mute").addClass("mejs-unmute")}else{d.setMuted(false);e.removeClass("mejs-unmute").addClass("mejs-mute")}p=Math.max(0,p);p=Math.min(p,1);d.setVolume(p)}},r=false;e.hover(function(){g.show()},function(){g.hide()});g.bind("mousedown",function(h){m(h);r=true;return false});f(document).bind("mouseup",function(){r=false}).bind("mousemove",function(h){r&&m(h)});e.find("button").click(function(){d.setMuted(!d.muted)});
...
for(i=0;i<a.text.length;i++){g.text[i]=j[i];g.times[i]={start:a.times[i].start,stop:a.times[i].stop,settings:a.times[i].settings}}e(g)})},translateText:function(a,c,b,d,e){for(var g,j=[],i,k="",l=function(){if(j.length>0){i=j.shift();mejs.TrackFormatParser.translateChunk(i,c,b,d,function(m){if(m!="undefined")k+=m;l()})}else e(k)};a.length>0;)if(a.length>1E3){g=a.lastIndexOf(".",1E3);j.push(a.substring(0,g));a=a.substring(g+1)}else{j.push(a);a=""}l()},translateChunk:function(a,c,b,d,e){a={q:a,langpair:c+
...
j;g++)if(e[g].isSeparator)d+='<div class="mejs-contextmenu-separator"></div>';else{var i=e[g].render(b);if(i!=null)d+='<div class="mejs-contextmenu-item" data-itemindex="'+g+'" id="element-'+Math.random()*1E6+'">'+i+"</div>"}b.contextMenu.empty().append(f(d)).css({top:c,left:a}).show();b.contextMenu.find(".mejs-contextmenu-item").each(function(){var k=f(this),l=parseInt(k.data("itemindex"),10),m=b.options.contextMenuItems[l];typeof m.show!="undefined"&&m.show(k,b);k.click(function(){typeof m.click!=
