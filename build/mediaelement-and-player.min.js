...
"");d.controls.css("pointer-events","");d.media.removeEventListener("click",d.clickToPlayPauseCallback);j=false}},s=f('<div class="mejs-fullscreen-hover" />').appendTo(d.container).mouseover(k).hide(),p=f('<div class="mejs-fullscreen-hover"  />').appendTo(d.container).mouseover(k).hide(),r=f('<div class="mejs-fullscreen-hover"  />').appendTo(d.container).mouseover(k).hide(),n=function(){var h={position:"absolute",top:0,left:0};s.css(h);p.css(h);r.css(h);s.width(d.container.width()).height(d.container.height()-
d.controls.height());h=g.offset().left-d.container.offset().left;fullScreenBtnWidth=g.outerWidth(true);p.width(h).height(d.controls.height()).css({top:d.container.height()-d.controls.height()});r.width(d.container.width()-h-fullScreenBtnWidth).height(d.controls.height()).css({top:d.container.height()-d.controls.height(),left:h+fullScreenBtnWidth})};d.globalBind("resize",function(){n()});g.mouseover(function(){if(!d.isFullScreen){var h=g.offset(),q=a.container.offset();e.positionFullscreenButton(h.left-
q.left,h.top-q.top,false);g.css("pointer-events","none");d.controls.css("pointer-events","none");d.media.addEventListener("click",d.clickToPlayPauseCallback);s.show();r.show();p.show();n();j=true}});e.addEventListener("fullscreenchange",function(){d.isFullScreen=!d.isFullScreen;d.isFullScreen?d.media.removeEventListener("click",d.clickToPlayPauseCallback):d.media.addEventListener("click",d.clickToPlayPauseCallback);k()})}else g.mouseover(function(){if(l!==null){clearTimeout(l);delete l}var h=g.offset(),
q=a.container.offset();e.positionFullscreenButton(h.left-q.left,h.top-q.top,true)}).mouseout(function(){if(l!==null){clearTimeout(l);delete l}l=setTimeout(function(){e.hideFullscreenButton()},1500)})}a.fullscreenBtn=g;d.globalBind("keydown",function(h){if((mejs.MediaFeatures.hasTrueNativeFullScreen&&mejs.MediaFeatures.isFullScreen()||d.isFullScreen)&&h.keyCode==27)a.exitFullScreen()})}},cleanfullscreen:function(a){a.exitFullScreen()},containerSizeTimeout:null,enterFullScreen:function(){var a=this;
if(!(a.media.pluginType!=="native"&&(mejs.MediaFeatures.isFirefox||a.options.usePluginFullScreen))){docStyleOverflow=document.documentElement.style.overflow;document.documentElement.style.overflow="hidden";normalHeight=a.container.height();normalWidth=a.container.width();if(a.media.pluginType==="native")if(mejs.MediaFeatures.hasTrueNativeFullScreen){mejs.MediaFeatures.requestFullScreen(a.container[0]);a.isInIframe&&setTimeout(function c(){if(a.isNativeFullScreen)f(window).width()!==screen.width?a.exitFullScreen():
