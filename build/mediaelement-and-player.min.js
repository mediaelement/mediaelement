...
this.media=this.$media[0];if(this.node.player)return this.node.player;else this.node.player=this;this.isVideo=this.media.tagName.toLowerCase()==="video";this.init();return this};mejs.MediaElementPlayer.prototype={init:function(){var a=this,c=mejs.MediaFeatures,b=f.extend(true,{},a.options,{success:function(d,e){a.meReady(d,e)},error:function(d){a.handleError(d)}});if(c.isiPad||c.isiPhone){a.$media.attr("controls","controls");a.$media.removeAttr("poster");if(c.isiPad&&a.media.getAttribute("autoplay")!==
...
(function(f){MediaElementPlayer.prototype.buildprogress=function(a,c,b,d){f('<div class="mejs-time-rail"><span class="mejs-time-total"><span class="mejs-time-loaded"></span><span class="mejs-time-current"></span><span class="mejs-time-handle"></span><span class="mejs-time-float"><span class="mejs-time-float-current">00:00</span><span class="mejs-time-float-corner"></span></span></span></div>').appendTo(c);var e=c.find(".mejs-time-total"),g=c.find(".mejs-time-loaded"),i=c.find(".mejs-time-current"),
k=c.find(".mejs-time-handle"),m=c.find(".mejs-time-float"),n=c.find(".mejs-time-float-current"),q=function(h){if(h){var l=h.target,p=null;if(l&&l.buffered&&l.buffered.length>0&&l.buffered.end&&l.duration)p=l.buffered.end(0)/l.duration;else if(l&&l.bytesTotal!=undefined&&l.bytesTotal>0&&l.bufferedBytes!=undefined)p=l.bufferedBytes/l.bytesTotal;else if(h.lengthComputable&&h.total!=0)p=h.loaded/h.total;if(p!==null){p=Math.min(1,Math.max(0,p));g.width(e.width()*p)}}},j=function(h){h=h.pageX;var l=e.offset(),
p=e.outerWidth(),s=0;s=0;if(h>l.left&&h<=p+l.left&&d.duration){s=(h-l.left)/p;s=s<=0.02?0:s*d.duration;r&&d.setCurrentTime(s);m.css("left",h-l.left);n.html(mejs.Utility.secondsToTimeCode(s))}},r=false,o=false;e.bind("mousedown",function(h){r=true;j(h);return false});c.find(".mejs-time-rail").bind("mouseenter",function(){o=true}).bind("mouseleave",function(){o=false});f(document).bind("mouseup",function(){r=false}).bind("mousemove",function(h){if(r||o)j(h)});d.addEventListener("progress",function(h){q(h)},
false);d.addEventListener("timeupdate",function(h){q(h);if(d.currentTime&&d.duration){h=e.width()*d.currentTime/d.duration;var l=h-k.outerWidth(true)/2;i.width(h);k.css("left",l)}},false)}})(jQuery);
...
(function(f){MediaElementPlayer.prototype.buildvolume=function(a,c,b,d){var e=f('<div class="mejs-button mejs-volume-button mejs-mute"><span></span><div class="mejs-volume-slider"><div class="mejs-volume-total"></div><div class="mejs-volume-current"></div><div class="mejs-volume-handle"></div></div></div>').appendTo(c);c=e.find(".mejs-volume-slider");var g=e.find(".mejs-volume-total"),i=e.find(".mejs-volume-current"),k=e.find(".mejs-volume-handle"),m=function(j){j=g.height()-g.height()*j;k.css("top",
j-k.height()/2);i.height(g.height()-j+parseInt(g.css("top").replace(/px/,""),10));i.css("top",j)},n=function(j){var r=g.height(),o=g.offset(),h=parseInt(g.css("top").replace(/px/,""),10);j=j.pageY-o.top;o=(r-j)/r;if(j<0)j=0;else if(j>r)j=r;k.css("top",j-k.height()/2+h);i.height(r-j);i.css("top",j+h);if(o==0){d.setMuted(true);e.removeClass("mejs-mute").addClass("mejs-unmute")}else{d.setMuted(false);e.removeClass("mejs-unmute").addClass("mejs-mute")}o=Math.max(0,o);o=Math.min(o,1);d.setVolume(o)},q=
false;c.bind("mousedown",function(j){n(j);q=true;return false});f(document).bind("mouseup",function(){q=false}).bind("mousemove",function(j){q&&n(j)});e.find("span").click(function(){if(d.muted){d.setMuted(false);e.removeClass("mejs-unmute").addClass("mejs-mute");m(1)}else{d.setMuted(true);e.removeClass("mejs-mute").addClass("mejs-unmute");m(0)}});d.addEventListener("volumechange",function(j){q||m(j.target.volume)},true);m(a.options.startVolume);d.setVolume(a.options.startVolume)}})(jQuery);
(function(f){MediaElementPlayer.prototype.buildfullscreen=function(a,c,b,d){if(a.isVideo){var e=0,g=0,i=a.container,k=f('<div class="mejs-button mejs-fullscreen-button"><span></span></div>').appendTo(c).click(function(){m(!d.isFullScreen)}),m=function(n){switch(d.pluginType){case "flash":case "silverlight":d.setFullscreen(n);break;case "native":if(mejs.MediaFeatures.hasNativeFullScreen)if(n){d.webkitEnterFullScreen();d.isFullScreen=true}else{d.webkitExitFullScreen();d.isFullScreen=false}else if(n){e=
a.$media.height();g=a.$media.width();i.addClass("mejs-container-fullscreen").width("100%").height("100%").css("z-index",1E3);a.$media.width("100%").height("100%");b.children("div").width("100%").height("100%");k.removeClass("mejs-fullscreen").addClass("mejs-unfullscreen");a.setControlsSize();d.isFullScreen=true}else{i.removeClass("mejs-container-fullscreen").width(g).height(e).css("z-index",1);a.$media.width(g).height(e);b.children("div").width(g).height(e);k.removeClass("mejs-unfullscreen").addClass("mejs-fullscreen");
a.setControlsSize();d.isFullScreen=false}}};f(document).bind("keydown",function(n){d.isFullScreen&&n.keyCode==27&&m(false)})}}})(jQuery);
...
c,b,d,function(m){i=m.split("<a></a>");for(k=0;k<a.text.length;k++){g.text[k]=i[k];g.times[k]={start:a.times[k].start,stop:a.times[k].stop,settings:a.times[k].settings}}e(g)})},translateText:function(a,c,b,d,e){for(var g,i=[],k,m="",n=function(){if(i.length>0){k=i.shift();mejs.SrtParser.translateChunk(k,c,b,d,function(q){if(q!="undefined")m+=q;n()})}else e(m)};a.length>0;)if(a.length>1E3){g=a.lastIndexOf(".",1E3);i.push(a.substring(0,g));a=a.substring(g+1)}else{i.push(a);a=""}n()},translateChunk:function(a,
