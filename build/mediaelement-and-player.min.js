...
d.addEventListener("waiting",function(){e.show()},false);d.addEventListener("loadeddata",function(){e.show()},false);d.addEventListener("canplay",function(){e.hide()},false);d.addEventListener("error",function(){e.hide();g.show();g.find("mejs-overlay-error").html("Error loading this resource")},false)}},buildkeyboard:function(a,b,c,d){f(document).keydown(function(e){if(a.hasFocus&&a.options.enableKeyboard)for(var g=0,l=a.options.keyActions.length;g<l;g++)for(var h=a.options.keyActions[g],i=0,j=h.keys.length;i<
j;i++)if(e.keyCode==h.keys[i]){e.preventDefault();h.action(a,d);return false}return true});f(document).click(function(e){if(f(e.target).closest(".mejs-container").length==0)a.hasFocus=false})},findTracks:function(){var a=this,b=a.$media.find("track");a.tracks=[];b.each(function(c,d){d=f(d);a.tracks.push({srclang:d.attr("srclang").toLowerCase(),src:d.attr("src"),kind:d.attr("kind"),label:d.attr("label")||"",entries:[],isLoaded:false})})},changeSkin:function(a){this.container[0].className="mejs-container "+
...
l=b.find(".mejs-time-handle"),h=b.find(".mejs-time-float"),i=b.find(".mejs-time-float-current"),j=function(m){m=m.pageX;var k=e.offset(),o=e.outerWidth(),n=0;n=0;var s=m-k.left;if(m>k.left&&m<=o+k.left&&d.duration){n=(m-k.left)/o;n=n<=0.02?0:n*d.duration;p&&d.setCurrentTime(n);if(!mejs.MediaFeatures.hasTouch){h.css("left",s);i.html(mejs.Utility.secondsToTimeCode(n));h.show()}}},p=false,r=false;e.bind("mousedown",function(m){if(m.which===1){p=true;j(m);return false}});b.find(".mejs-time-total").bind("mouseenter",
...
(function(f){f.extend(mejs.MepDefaults,{duration:-1});f.extend(MediaElementPlayer.prototype,{buildcurrent:function(a,b,c,d){f('<div class="mejs-time"><span class="mejs-currenttime">'+this.secondsToTimeCode(0)+"</span></div>").appendTo(b);this.currenttime=this.controls.find(".mejs-currenttime");d.addEventListener("timeupdate",function(){a.updateCurrent()},false)},buildduration:function(a,b,c,d){if(b.children().last().find(".mejs-currenttime").length>0)f(' <span> | </span> <span class="mejs-duration">'+
(this.options.duration>0?mejs.Utility.secondsToTimeCode(this.options.duration,this.options.alwaysShowHours||this.media.duration>3600,this.options.showTimecodeFrameCount,this.options.framesPerSecond||25):(a.options.alwaysShowHours?"00:":"")+(a.options.showTimecodeFrameCount?"00:00:00":"00:00"))+"</span>").appendTo(b.find(".mejs-time"));else{b.find(".mejs-currenttime").parent().addClass("mejs-currenttime-container");f('<div class="mejs-time mejs-duration-container"><span class="mejs-duration">'+(this.options.duration>
0?mejs.Utility.secondsToTimeCode(this.options.duration,this.options.alwaysShowHours||this.media.duration>3600,this.options.showTimecodeFrameCount,this.options.framesPerSecond||25):(a.options.alwaysShowHours?"00:":"")+(a.options.showTimecodeFrameCount?"00:00:00":"00:00"))+"</span></div>").appendTo(b)}this.durationD=this.controls.find(".mejs-duration");d.addEventListener("timeupdate",function(){a.updateDuration()},false)},updateCurrent:function(){this.currenttime&&this.currenttime.html(this.secondsToTimeCode(this.media.currentTime))},
updateDuration:function(){if(this.media.duration&&this.durationD)this.durationD.html(mejs.Utility.secondsToTimeCode(this.media.duration,this.options.alwaysShowHours,this.options.showTimecodeFrameCount,this.options.framesPerSecond||25))},secondsToTimeCode:function(a){if(typeof this.timecodeStartComps=="undefined")this.timecodeStartComps=typeof this.options.timecodeStart!="undefined"?f.map(this.options.timecodeStart.split(":"),function(c){return+c}):[0,0,0,0];if(a<0)return"--:--:--";var b=[];b[0]=(Math.floor(a/
3600)+this.timecodeStartComps[0]<10?"0":"")+(Math.floor(a/3600)+this.timecodeStartComps[0]);b[1]=(Math.floor(a%3600/60)+this.timecodeStartComps[1]<10?"0":"")+(Math.floor(a%3600/60)+this.timecodeStartComps[1]);b[2]=(Math.floor(a%60)+this.timecodeStartComps[2]<10?"0":"")+(Math.floor(a%60)+this.timecodeStartComps[2]);return b.join(":")}})})(mejs.$);
...
else{var l=null;if(document.documentElement.style.pointerEvents===""){var h=false;g.mouseover(function(){if(!e.isFullScreen){var i=g.offset(),j=a.container.offset();d.positionFullscreenButton(i.left-j.left,i.top-j.top,false);console.log("killing pointer");g.css("pointer-events","none");e.controls.css("pointer-events","none");h=true}});f(document).mousemove(function(i){if(h){var j=g.offset();console.log(j,i.pageY,i.pageX);if(i.pageY<j.top||i.pageY>j.top+g.outerHeight(true)||i.pageX<j.left||i.pageX>
j.left+g.outerWidth(true)){g.css("pointer-events","");e.controls.css("pointer-events","");h=false;console.log("restored pointer")}}})}else g.mouseover(function(){if(l!==null){clearTimeout(l);delete l}var i=g.offset(),j=a.container.offset();d.positionFullscreenButton(i.left-j.left,i.top-j.top,true)}).mouseout(function(){if(l!==null){clearTimeout(l);delete l}l=setTimeout(function(){d.hideFullscreenButton()},1500)})}a.fullscreenBtn=g;f(document).bind("keydown",function(i){if((mejs.MediaFeatures.hasTrueNativeFullScreen&&
mejs.MediaFeatures.isFullScreen()||e.isFullScreen)&&i.keyCode==27)a.exitFullScreen()})}},enterFullScreen:function(){var a=this;if(!(a.media.pluginType!=="native"&&(mejs.MediaFeatures.isFirefox||a.options.usePluginFullScreen))){docStyleOverflow=document.documentElement.style.overflow;document.documentElement.style.overflow="hidden";normalHeight=a.container.height();normalWidth=a.container.width();if(a.media.pluginType==="native")if(mejs.MediaFeatures.hasTrueNativeFullScreen)mejs.MediaFeatures.requestFullScreen(a.container[0]);
else if(mejs.MediaFeatures.hasSemiNativeFullScreen){a.media.webkitEnterFullscreen();return}if(a.isInIframe){var b=a.options.newWindowCallback(this);if(b!=="")if(mejs.MediaFeatures.hasTrueNativeFullScreen)setTimeout(function(){if(!a.isNativeFullScreen){a.pause();window.open(b,a.id,"top=0,left=0,width="+screen.availWidth+",height="+screen.availHeight+",resizable=yes,scrollbars=no,status=no,toolbar=no")}},250);else{a.pause();window.open(b,a.id,"top=0,left=0,width="+screen.availWidth+",height="+screen.availHeight+
...
l;g++)if(e[g].isSeparator)d+='<div class="mejs-contextmenu-separator"></div>';else{var h=e[g].render(c);if(h!=null)d+='<div class="mejs-contextmenu-item" data-itemindex="'+g+'" id="element-'+Math.random()*1E6+'">'+h+"</div>"}c.contextMenu.empty().append(f(d)).css({top:b,left:a}).show();c.contextMenu.find(".mejs-contextmenu-item").each(function(){var i=f(this),j=parseInt(i.data("itemindex"),10),p=c.options.contextMenuItems[j];typeof p.show!="undefined"&&p.show(i,c);i.click(function(){typeof p.click!=
