...
k=c.find(".mejs-time-handle"),i=c.find(".mejs-time-float"),m=c.find(".mejs-time-float-current"),l=function(h){h=h.pageX;var j=e.offset(),p=e.outerWidth(),o=0;o=0;var s=h-j.left;if(h>j.left&&h<=p+j.left&&d.duration){o=(h-j.left)/p;o=o<=0.02?0:o*d.duration;n&&d.setCurrentTime(o);if(!mejs.MediaFeatures.hasTouch){i.css("left",s);m.html(mejs.Utility.secondsToTimeCode(o));i.show()}}},n=false,q=false;e.bind("mousedown",function(h){if(h.which===1){n=true;l(h);return false}});c.find(".mejs-time-total").bind("mouseenter",
function(){q=true;mejs.MediaFeatures.hasTouch||i.show()}).bind("mouseleave",function(){q=false;i.hide()});f(document).bind("mouseup",function(){n=false;i.hide()}).bind("mousemove",function(h){if(n||q)l(h)});d.addEventListener("progress",function(h){a.setProgressRail(h);a.setCurrentRail(h)},false);d.addEventListener("timeupdate",function(h){a.setProgressRail(h);a.setCurrentRail(h)},false);this.loaded=b;this.total=e;this.current=g;this.handle=k},setProgressRail:function(a){var c=a!=undefined?a.target:
...
g=e.find(".mejs-volume-slider"),k=e.find(".mejs-volume-total"),i=e.find(".mejs-volume-current"),m=e.find(".mejs-volume-handle"),l=function(j){if(g.is(":visible")){var p=k.height(),o=k.position();j=p-p*j;m.css("top",o.top+j-m.height()/2);i.height(p-j);i.css("top",o.top+j)}else{g.show();l(j);g.hide()}},n=function(j){var p=k.height(),o=k.offset(),s=parseInt(k.css("top").replace(/px/,""),10);j=j.pageY-o.top;var r=(p-j)/p;if(!(o.top==0||o.left==0)){r=Math.max(0,r);r=Math.min(r,1);if(j<0)j=0;else if(j>
p)j=p;m.css("top",j-m.height()/2+s);i.height(p-j);i.css("top",j+s);if(r==0){d.setMuted(true);e.removeClass("mejs-mute").addClass("mejs-unmute")}else{d.setMuted(false);e.removeClass("mejs-unmute").addClass("mejs-mute")}r=Math.max(0,r);r=Math.min(r,1);d.setVolume(r)}},q=false,h=false;e.hover(function(){g.show();h=true},function(){h=false;q||g.hide()});g.bind("mouseover",function(){h=true}).bind("mousedown",function(j){n(j);q=true;return false});f(document).bind("mouseup",function(){q=false;h||g.hide()}).bind("mousemove",
function(j){q&&n(j)});e.find("button").click(function(){d.setMuted(!d.muted)});d.addEventListener("volumechange",function(){if(!q)if(d.muted){l(0);e.removeClass("mejs-mute").addClass("mejs-unmute")}else{l(d.volume);e.removeClass("mejs-unmute").addClass("mejs-mute")}},false);l(a.options.startVolume);d.pluginType==="native"&&d.setVolume(a.options.startVolume)}}})})(mejs.$);
...
else{var k=null;if(document.documentElement.style.pointerEvents===""&&!mejs.MediaFeatures.isOpera){var i=false,m=function(){if(i){l.hide();n.hide();g.css("pointer-events","");e.controls.css("pointer-events","");i=false}},l=f("<div />").appendTo(e.container).mouseover(m),n=f("<div />").appendTo(e.container).mouseover(m),q=function(){var h={position:"absolute",top:0,left:0};l.css(h);n.css(h);l.width(e.container.width()).height(e.container.height()-e.controls.height());n.width(e.container.width()-g.width()).height(e.controls.height()).css({top:e.container.height()-
e.controls.height()})};f(document).resize(function(){q()});g.mouseover(function(){if(!e.isFullScreen){var h=g.offset(),j=a.container.offset();d.positionFullscreenButton(h.left-j.left,h.top-j.top,false);g.css("pointer-events","none");e.controls.css("pointer-events","none");l.show();n.show();q();i=true}});d.addEventListener("fullscreenchange",function(){m()})}else g.mouseover(function(){if(k!==null){clearTimeout(k);delete k}var h=g.offset(),j=a.container.offset();d.positionFullscreenButton(h.left-j.left,
h.top-j.top,true)}).mouseout(function(){if(k!==null){clearTimeout(k);delete k}k=setTimeout(function(){d.hideFullscreenButton()},1500)})}a.fullscreenBtn=g;f(document).bind("keydown",function(h){if((mejs.MediaFeatures.hasTrueNativeFullScreen&&mejs.MediaFeatures.isFullScreen()||e.isFullScreen)&&h.keyCode==27)a.exitFullScreen()})}},enterFullScreen:function(){var a=this;if(!(a.media.pluginType!=="native"&&(mejs.MediaFeatures.isFirefox||a.options.usePluginFullScreen))){docStyleOverflow=document.documentElement.style.overflow;
document.documentElement.style.overflow="hidden";normalHeight=a.container.height();normalWidth=a.container.width();if(a.media.pluginType==="native")if(mejs.MediaFeatures.hasTrueNativeFullScreen)mejs.MediaFeatures.requestFullScreen(a.container[0]);else if(mejs.MediaFeatures.hasSemiNativeFullScreen){a.media.webkitEnterFullscreen();return}if(a.isInIframe){var c=a.options.newWindowCallback(this);if(c!=="")if(mejs.MediaFeatures.hasTrueNativeFullScreen)setTimeout(function(){if(!a.isNativeFullScreen){a.pause();
