...
controlsAreVisible:true,init:function(){var a=this,b=mejs.MediaFeatures,c=f.extend(true,{},a.options,{success:function(e,h){a.meReady(e,h)},error:function(e){a.handleError(e)}}),d=a.media.tagName.toLowerCase();a.isDynamic=d!=="audio"&&d!=="video";a.isVideo=a.isDynamic?a.options.isVideo:d!=="audio"&&a.options.isVideo;if(b.isiPad&&a.options.iPadUseNativeControls||b.isiPhone&&a.options.iPhoneUseNativeControls){a.$media.attr("controls","controls");if(b.isiPad&&a.media.getAttribute("autoplay")!==null){a.media.load();
...
e==="false");var h;if(!c.created){c.created=true;c.media=a;c.domNode=b;if(!(d.isAndroid&&c.options.AndroidUseNativeControls)&&!(d.isiPad&&c.options.iPadUseNativeControls)&&!(d.isiPhone&&c.options.iPhoneUseNativeControls)){c.buildposter(c,c.controls,c.layers,c.media);c.buildkeyboard(c,c.controls,c.layers,c.media);c.buildoverlays(c,c.controls,c.layers,c.media);c.findTracks();for(h in c.options.features){d=c.options.features[h];if(c["build"+d])try{c["build"+d](c,c.controls,c.layers,c.media)}catch(m){}}c.container.trigger("controlsready");
...
0)c=f('<img width="100%" height="100%" />').appendTo(b);c.attr("src",a);this.$node.attr("poster",a)},buildoverlays:function(a,b,c,d){var e=this;if(a.isVideo){var h=f('<div class="mejs-overlay mejs-layer"><div class="mejs-overlay-loading"><span></span></div></div>').hide().appendTo(c),m=f('<div class="mejs-overlay mejs-layer"><div class="mejs-overlay-error"></div></div>').hide().appendTo(c),g=f('<div class="mejs-overlay mejs-layer mejs-overlay-play"><div class="mejs-overlay-button"></div></div>').appendTo(c).click(function(){if(e.options.clickToPlayPause)d.paused?
d.play():d.pause()});d.addEventListener("play",function(){g.hide();h.hide();b.find(".mejs-time-buffering").hide();m.hide()},false);d.addEventListener("playing",function(){g.hide();h.hide();b.find(".mejs-time-buffering").hide();m.hide()},false);d.addEventListener("seeking",function(){h.show();b.find(".mejs-time-buffering").show()},false);d.addEventListener("seeked",function(){h.hide();b.find(".mejs-time-buffering").hide()},false);d.addEventListener("pause",function(){mejs.MediaFeatures.isiPhone||g.show()},
false);d.addEventListener("waiting",function(){h.show();b.find(".mejs-time-buffering").show()},false);d.addEventListener("loadeddata",function(){h.show();b.find(".mejs-time-buffering").show()},false);d.addEventListener("canplay",function(){h.hide();b.find(".mejs-time-buffering").hide()},false);d.addEventListener("error",function(){h.hide();b.find(".mejs-time-buffering").hide();m.show();m.find("mejs-overlay-error").html("Error loading this resource")},false)}},buildkeyboard:function(a,b,c,d){f(document).keydown(function(e){if(a.hasFocus&&
a.options.enableKeyboard)for(var h=0,m=a.options.keyActions.length;h<m;h++)for(var g=a.options.keyActions[h],j=0,p=g.keys.length;j<p;j++)if(e.keyCode==g.keys[j]){e.preventDefault();g.action(a,d,e.keyCode);return false}return true});f(document).click(function(e){if(f(e.target).closest(".mejs-container").length==0)a.hasFocus=false})},findTracks:function(){var a=this,b=a.$media.find("track");a.tracks=[];b.each(function(c,d){d=f(d);a.tracks.push({srclang:d.attr("srclang").toLowerCase(),src:d.attr("src"),
...
(function(f){f.extend(mejs.MepDefaults,{playpauseText:"Play/Pause"});f.extend(MediaElementPlayer.prototype,{buildplaypause:function(a,b,c,d){var e=f('<div class="mejs-button mejs-playpause-button mejs-play" ><button type="button" aria-controls="'+this.id+'" title="'+this.options.playpauseText+'"></button></div>').appendTo(b).click(function(h){h.preventDefault();d.paused?d.play():d.pause();return false});d.addEventListener("play",function(){e.removeClass("mejs-play").addClass("mejs-pause")},false);
...
b.find(".mejs-time-total");c=b.find(".mejs-time-loaded");var h=b.find(".mejs-time-current"),m=b.find(".mejs-time-handle"),g=b.find(".mejs-time-float"),j=b.find(".mejs-time-float-current"),p=function(k){k=k.pageX;var q=e.offset(),n=e.outerWidth(true),l=0,o=l=0;if(d.duration){if(k<q.left)k=q.left;else if(k>n+q.left)k=n+q.left;o=k-q.left;l=o/n;l=l<=0.02?0:l*d.duration;i&&l!==d.currentTime&&d.setCurrentTime(l);if(!mejs.MediaFeatures.hasTouch){g.css("left",o);j.html(mejs.Utility.secondsToTimeCode(l));
g.show()}}},i=false;e.bind("mousedown",function(k){if(k.which===1){i=true;p(k);f(document).bind("mousemove.dur",function(q){p(q)}).bind("mouseup.dur",function(){i=false;g.hide();f(document).unbind(".dur")});return false}}).bind("mouseenter",function(){f(document).bind("mousemove.dur",function(k){p(k)});mejs.MediaFeatures.hasTouch||g.show()}).bind("mouseleave",function(){if(!i){f(document).unbind(".dur");g.hide()}});d.addEventListener("progress",function(k){a.setProgressRail(k);a.setCurrentRail(k)},
false);d.addEventListener("timeupdate",function(k){a.setProgressRail(k);a.setCurrentRail(k)},false);this.loaded=c;this.total=e;this.current=h;this.handle=m},setProgressRail:function(a){var b=a!=undefined?a.target:this.media,c=null;if(b&&b.buffered&&b.buffered.length>0&&b.buffered.end&&b.duration)c=b.buffered.end(0)/b.duration;else if(b&&b.bytesTotal!=undefined&&b.bytesTotal>0&&b.bufferedBytes!=undefined)c=b.bufferedBytes/b.bytesTotal;else if(a&&a.lengthComputable&&a.total!=0)c=a.loaded/a.total;if(c!==
...
(function(f){f.extend(mejs.MepDefaults,{muteText:"Mute Toggle",hideVolumeOnTouchDevices:true,audioVolume:"horizontal",videoVolume:"vertical"});f.extend(MediaElementPlayer.prototype,{buildvolume:function(a,b,c,d){if(!(mejs.MediaFeatures.hasTouch&&this.options.hideVolumeOnTouchDevices)){var e=this.isVideo?this.options.videoVolume:this.options.audioVolume,h=e=="horizontal"?f('<div class="mejs-button mejs-volume-button mejs-mute"><button type="button" aria-controls="'+this.id+'" title="'+this.options.muteText+
'"></button></div><div class="mejs-horizontal-volume-slider"><div class="mejs-horizontal-volume-total"></div><div class="mejs-horizontal-volume-current"></div><div class="mejs-horizontal-volume-handle"></div></div>').appendTo(b):f('<div class="mejs-button mejs-volume-button mejs-mute"><button type="button" aria-controls="'+this.id+'" title="'+this.options.muteText+'"></button><div class="mejs-volume-slider"><div class="mejs-volume-total"></div><div class="mejs-volume-current"></div><div class="mejs-volume-handle"></div></div></div>').appendTo(b),
m=this.container.find(".mejs-volume-slider, .mejs-horizontal-volume-slider"),g=this.container.find(".mejs-volume-total, .mejs-horizontal-volume-total"),j=this.container.find(".mejs-volume-current, .mejs-horizontal-volume-current"),p=this.container.find(".mejs-volume-handle, .mejs-horizontal-volume-handle"),i=function(l,o){if(!m.is(":visible")&&typeof o=="undefined"){m.show();i(l,true);m.hide()}else{l=Math.max(0,l);l=Math.min(l,1);l==0?h.removeClass("mejs-mute").addClass("mejs-unmute"):h.removeClass("mejs-unmute").addClass("mejs-mute");
if(e=="vertical"){var r=g.height(),s=g.position(),t=r-r*l;p.css("top",Math.round(s.top+t-p.height()/2));j.height(r-t);j.css("top",s.top+t)}else{r=g.width();s=g.position();r=r*l;p.css("left",Math.round(s.left+r-p.width()/2));j.width(Math.round(r))}}},k=function(l){var o=null,r=g.offset();if(e=="vertical"){o=g.height();parseInt(g.css("top").replace(/px/,""),10);o=(o-(l.pageY-r.top))/o;if(r.top==0||r.left==0)return}else{o=g.width();o=(l.pageX-r.left)/o}o=Math.max(0,o);o=Math.min(o,1);i(o);o==0?d.setMuted(true):
d.setMuted(false);d.setVolume(o)},q=false,n=false;h.hover(function(){m.show();n=true},function(){n=false;!q&&e=="vertical"&&m.hide()});m.bind("mouseover",function(){n=true}).bind("mousedown",function(l){k(l);f(document).bind("mousemove.vol",function(o){k(o)}).bind("mouseup.vol",function(){q=false;f(document).unbind(".vol");!n&&e=="vertical"&&m.hide()});q=true;return false});h.find("button").click(function(){d.setMuted(!d.muted)});d.addEventListener("volumechange",function(){if(!q)if(d.muted){i(0);
h.removeClass("mejs-mute").addClass("mejs-unmute")}else{i(d.volume);h.removeClass("mejs-unmute").addClass("mejs-mute")}},false);if(this.container.is(":visible")){i(a.options.startVolume);d.pluginType==="native"&&d.setVolume(a.options.startVolume)}}}})})(mejs.$);
...
function(){if(mejs.MediaFeatures.isFullScreen()){a.isNativeFullScreen=true;a.setControlsSize()}else{a.isNativeFullScreen=false;a.exitFullScreen()}})}var e=this,h=f('<div class="mejs-button mejs-fullscreen-button"><button type="button" aria-controls="'+e.id+'" title="'+e.options.fullscreenText+'"></button></div>').appendTo(b);if(e.media.pluginType==="native"||!e.options.usePluginFullScreen&&!mejs.MediaFeatures.isFirefox)h.click(function(){mejs.MediaFeatures.hasTrueNativeFullScreen&&mejs.MediaFeatures.isFullScreen()||
a.isFullScreen?a.exitFullScreen():a.enterFullScreen()});else{var m=null;if(function(){var n=document.createElement("x"),l=document.documentElement,o=window.getComputedStyle;if(!("pointerEvents"in n.style))return false;n.style.pointerEvents="auto";n.style.pointerEvents="x";l.appendChild(n);o=o&&o(n,"").pointerEvents==="auto";l.removeChild(n);return!!o}()&&!mejs.MediaFeatures.isOpera){var g=false,j=function(){if(g){p.hide();i.hide();k.hide();h.css("pointer-events","");e.controls.css("pointer-events",
"");g=false}},p=f('<div class="mejs-fullscreen-hover" />').appendTo(e.container).mouseover(j),i=f('<div class="mejs-fullscreen-hover"  />').appendTo(e.container).mouseover(j),k=f('<div class="mejs-fullscreen-hover"  />').appendTo(e.container).mouseover(j),q=function(){var n={position:"absolute",top:0,left:0};p.css(n);i.css(n);k.css(n);p.width(e.container.width()).height(e.container.height()-e.controls.height());n=h.offset().left-e.container.offset().left;fullScreenBtnWidth=h.outerWidth(true);i.width(n).height(e.controls.height()).css({top:e.container.height()-
e.controls.height()});k.width(e.container.width()-n-fullScreenBtnWidth).height(e.controls.height()).css({top:e.container.height()-e.controls.height(),left:n+fullScreenBtnWidth})};f(document).resize(function(){q()});h.mouseover(function(){if(!e.isFullScreen){var n=h.offset(),l=a.container.offset();d.positionFullscreenButton(n.left-l.left,n.top-l.top,false);h.css("pointer-events","none");e.controls.css("pointer-events","none");p.show();k.show();i.show();q();g=true}});d.addEventListener("fullscreenchange",
function(){j()})}else h.mouseover(function(){if(m!==null){clearTimeout(m);delete m}var n=h.offset(),l=a.container.offset();d.positionFullscreenButton(n.left-l.left,n.top-l.top,true)}).mouseout(function(){if(m!==null){clearTimeout(m);delete m}m=setTimeout(function(){d.hideFullscreenButton()},1500)})}a.fullscreenBtn=h;f(document).bind("keydown",function(n){if((mejs.MediaFeatures.hasTrueNativeFullScreen&&mejs.MediaFeatures.isFullScreen()||e.isFullScreen)&&n.keyCode==27)a.exitFullScreen()})}},enterFullScreen:function(){var a=
...
stop:mejs.Utility.convertSMPTEtoSeconds(d[3]),settings:d[5]})}}return c}},dfxp:{parse:function(a){a=f(a).filter("tt");var b=0;b=a.children("div").eq(0);var c=b.find("p");b=a.find("#"+b.attr("style"));var d,e;a={text:[],times:[]};if(b.length){e=b.removeAttr("id").get(0).attributes;if(e.length){d={};for(b=0;b<e.length;b++)d[e[b].name.split(":")[1]]=e[b].value}}for(b=0;b<c.length;b++){var h;e={start:null,stop:null,style:null};if(c.eq(b).attr("begin"))e.start=mejs.Utility.convertSMPTEtoSeconds(c.eq(b).attr("begin"));
if(!e.start&&c.eq(b-1).attr("end"))e.start=mejs.Utility.convertSMPTEtoSeconds(c.eq(b-1).attr("end"));if(c.eq(b).attr("end"))e.stop=mejs.Utility.convertSMPTEtoSeconds(c.eq(b).attr("end"));if(!e.stop&&c.eq(b+1).attr("begin"))e.stop=mejs.Utility.convertSMPTEtoSeconds(c.eq(b+1).attr("begin"));if(d){h="";for(var m in d)h+=m+":"+d[m]+";"}if(h)e.style=h;if(e.start==0)e.start=0.2;a.times.push(e);e=f.trim(c.eq(b).html()).replace(/(\b(https?|ftp|file):\/\/[-A-Z0-9+&@#\/%?=~_|!:,.;]*[-A-Z0-9+&@#\/%=~_|])/ig,
...
true},disableContextMenu:function(){this.isContextMenuEnabled=false},contextMenuTimeout:null,startContextMenuTimer:function(){var a=this;a.killContextMenuTimer();a.contextMenuTimer=setTimeout(function(){a.hideContextMenu();a.killContextMenuTimer()},750)},killContextMenuTimer:function(){var a=this.contextMenuTimer;if(a!=null){clearTimeout(a);delete a}},hideContextMenu:function(){this.contextMenu.hide()},renderContextMenu:function(a,b){for(var c=this,d="",e=c.options.contextMenuItems,h=0,m=e.length;h<
m;h++)if(e[h].isSeparator)d+='<div class="mejs-contextmenu-separator"></div>';else{var g=e[h].render(c);if(g!=null)d+='<div class="mejs-contextmenu-item" data-itemindex="'+h+'" id="element-'+Math.random()*1E6+'">'+g+"</div>"}c.contextMenu.empty().append(f(d)).css({top:b,left:a}).show();c.contextMenu.find(".mejs-contextmenu-item").each(function(){var j=f(this),p=parseInt(j.data("itemindex"),10),i=c.options.contextMenuItems[p];typeof i.show!="undefined"&&i.show(j,c);j.click(function(){typeof i.click!=
"undefined"&&i.click(c);c.contextMenu.hide()})});setTimeout(function(){c.killControlsTimer("rev3")},100)}})})(mejs.$);
...
(function(f){f.extend(MediaElementPlayer.prototype,{buildplaylist:function(a,b,c,d){var e=false,h=f('<div class="mejs-playlist"><div class="mejs-block"><div class="mejs-button mejs-close"><button type="button"></button></div><nav class="mejs-playlist-paging"></nav><div class="mejs-playlist-content"></div></div></div>').appendTo(c).on("click",".mejs-playlist-content a[href]",function(g){g.preventDefault();g=f(g.currentTarget).closest(".mejs-playlist-content").find("li a").index(g.currentTarget);a.setItem(g);
a.isVideo&&a.closePlaylistPanel();return false});h.find(".mejs-close").click(function(){a.closePlaylistPanel()});var m=f('<div class="mejs-button mejs-playlist-openclose mejs-playlist-open" type="button"><button type="button"></button></div>').appendTo(b).mouseenter(function(){a.openPlaylistPanel()}).click(function(){e?a.closePlaylistPanel():a.openPlaylistPanel()});this.openPlaylistPanel=function(){if(!e){h.slideDown(function(){e=true;if(a.isVideo){var g=h.find(".mejs-playlist-paging"),j=h.find(".mejs-playlist-content ol li"),
p=Math.floor(h.width()/j.width()),i=Math.floor(h.height()/j.height()),k=Math.ceil(j.length/(p*i)),q=1,n=document.createElement("ul");g.empty().append(n);if(k>1){for(var l=1;l<=k;l++){var o=document.createElement("li");o.innerHTML=l;o.className="mejs-page";n.appendChild(o)}l=document.createElement("button");l.innerHTML="Next page";l.className="mejs-page-next";o=document.createElement("button");o.innerHTML="Previous page";o.className="mejs-page-prev";n.appendChild(o);n.appendChild(l);g.find(".mejs-page").click(function(r){$e=
f(r.currentTarget);q=$e.index()+1;j.parent().css("top",(-h.height()+parseInt(j.parent().css("margin-top"),10))*$e.index());$e.siblings().andSelf().removeClass("mejs-page-current");$e.addClass("mejs-page-current")});f(o).click(function(){q>1&&g.find(".mejs-page").eq(q-1-1).click()});f(l).click(function(){q<k&&g.find(".mejs-page").eq(q-1+1).click()});g.find(".mejs-page").eq(Math.floor(a.playlist.current/(p*i))).click()}}});f(m).addClass("mejs-playlist-close").removeClass("mejs-playlist-open")}};this.closePlaylistPanel=
function(){if(e){h.slideUp(function(){e=false});f(m).addClass("mejs-playlist-open").removeClass("mejs-playlist-close")}};b=d.getAttribute("title");if(!b){b=d.src.split("/");b=b[b.length-1]}d={src:d.src,title:b,poster:d.getAttribute("poster")};b=a.$media.find('meta[itemprop="url"]');if(b.length)d.url=b.attr("content");a.loadPlaylist([d])},loadPlaylist:function(a){function b(){var i,k;if(e.length){i=c.playlist.current;for(k=true;i>0;){i--;if(!(c.playlist.list[i].src==undefined||c.playlist.list[i].src==
"")){k=false;break}}k?e.attr("disabled","disabled"):e.removeAttr("disabled")}if(h.length){i=c.playlist.current;for(k=true;i+1<c.playlist.list.length;){i++;if(!(c.playlist.list[i].src==undefined||c.playlist.list[i].src=="")){k=false;break}}k?h.attr("disabled","disabled"):h.removeAttr("disabled")}}var c=this,d=this.container.find(".mejs-playlist-content"),e=this.container.find(".mejs-prev button"),h=this.container.find(".mejs-next button");this.playlist={list:[],current:-1};d.length&&d.empty();for(var m=
1,g=0;g<a.length;g++){var j={};if(a[g].src)j.src=a[g].src;if(a[g].poster)j.poster=a[g].poster;j.title=a[g].title?a[g].title:"";if(a[g].disk){j.disk=a[g].disk;m=1}j.nb=a[g].nb?a[g].nb:m++;if(a[g].url)j.url=a[g].url;this.playlist.list.push(j);if(d.length){var p='<span class="mejs-playlist-nb">'+j.nb+".</span>";if(j.poster&&c.isVideo)p+='<span class="mejs-playlist-poster"><img src="'+j.poster+'"/></span>';p+='<span class="mejs-playlist-title">'+j.title+"</span>";if(j.url)p+='<meta itemprop="url" content="'+
j.url+'" />';$entry=j.src?f('<li><a href="'+j.src+'">'+p+"</a></li>"):f("<li><a>"+p+"</a></li>");if(d.find("ol").length==0||j.disk)d.append(document.createElement("ol"));j.disk&&d.find("ol:last-child").append('<li class="mejs-playlist-disk">'+j.disk+"</li>");d.find("ol:last-child").append($entry)}}this.setTrackTitle&&this.setTrackTitle(this.playlist.list[0].title);this.setPoster(this.playlist.list[0].poster);this.setItem=function(i){if(!(this.playlist.list[i].src==undefined||this.playlist.list[i].src==
"")){this.playlist.current=i;this.pause();this.setSrc(this.playlist.list[i].src);this.load();this.setTrackTitle&&this.setTrackTitle(this.playlist.list[i].title);this.setPoster(this.playlist.list[i].poster);var k=c.$media.find('meta[itemprop="url"]');if(k.length)this.playlist.list[i].url?k.attr("content",this.playlist.list[i].url):k.remove();else this.playlist.list[i].url&&f(document.createElement("meta")).attr("itemprop","url").attr("content",this.playlist.list[i].url).appendTo(c.$media);b();return this}};
this.setPrevItem=function(){for(var i=this.playlist.current;i>0;){i--;if(!(this.playlist.list[i].src==undefined||this.playlist.list[i].src=="")){this.setItem(i);break}}return this};this.setNextItem=function(){for(var i=this.playlist.current;i+1<this.playlist.list.length;){i++;if(!(this.playlist.list[i].src==undefined||this.playlist.list[i].src=="")){this.setItem(i);break}}return this};this.playlist.alreadyAttached||c.media.addEventListener("ended",function(){var i=c.playlist.current;c.pause();c.setNextItem();
c.playlist.current!=i&&setTimeout(function(){c.load();c.play()},0)},false);this.playlist.alreadyAttached=true;b();this.setNextItem();return this},buildprev:function(a,b){f('<div class="mejs-button mejs-prev" type="button"><button type="button" disabled></button></div>').appendTo(b).click(function(c){c.preventDefault();a.setPrevItem();return false})},buildnext:function(a,b){f('<div class="mejs-button mejs-next" type="button"><button type="button" disabled></button></div>').appendTo(b).click(function(c){c.preventDefault();
a.setNextItem();return false})}})})(mejs.$);
(function(f){f.extend(MediaElementPlayer.prototype,{buildtitle:function(a,b,c,d){f('<div class="mejs-title"><div class="mejs-title-playlist"></div><div class="mejs-title-track"></div></div>').appendTo(c);b=a.$media.attr("title");if(a.options.title!=undefined)b=a.options.title;else if(b==undefined){d=d.src.split("/");b=d[d.length-1]}a.setTrackTitle(b)},setTrackTitle:function(a){this.container.find(".mejs-title-track").text(a);return this},setPlaylistTitle:function(a){this.container.find(".mejs-title-playlist").text(a);return this}})})(mejs.$);
