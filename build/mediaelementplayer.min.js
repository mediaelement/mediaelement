...
<<<<<<< HEAD
g.show();g.find("mejs-overlay-error").html("Error loading this resource")},false)}},buildkeyboard:function(a,b,c,e){f(document).keydown(function(d){if(a.hasFocus&&a.options.enableKeyboard)for(var g=0,k=a.options.keyActions.length;g<k;g++)for(var h=a.options.keyActions[g],l=0,n=h.keys.length;l<n;l++)if(d.keyCode==h.keys[l]){d.preventDefault();h.action(a,e,d.keyCode);return false}return true});f(document).click(function(d){if(f(d.target).closest(".mejs-container").length==0)a.hasFocus=false})},findTracks:function(){var a=
=======
...
<<<<<<< HEAD
b.find(".mejs-time-total");c=b.find(".mejs-time-loaded");var g=b.find(".mejs-time-current"),k=b.find(".mejs-time-handle"),h=b.find(".mejs-time-float"),l=b.find(".mejs-time-float-current"),n=function(j){j=j.pageX;var s=d.offset(),i=d.outerWidth(),o=0;o=0;var m=j-s.left;if(j>s.left&&j<=i+s.left&&e.duration){o=(j-s.left)/i;o=o<=0.02?0:o*e.duration;p&&e.setCurrentTime(o);if(!mejs.MediaFeatures.hasTouch){h.css("left",m);l.html(mejs.Utility.secondsToTimeCode(o));h.show()}}},p=false;d.bind("mousedown",function(j){if(j.which===
1){p=true;n(j);f(document).bind("mousemove.dur",function(s){n(s)}).bind("mouseup.dur",function(){p=false;h.hide();f(document).unbind(".dur")});return false}}).bind("mouseenter",function(){f(document).bind("mousemove.dur",function(j){n(j)});mejs.MediaFeatures.hasTouch||h.show()}).bind("mouseleave",function(){if(!p){f(document).unbind(".dur");h.hide()}});e.addEventListener("progress",function(j){a.setProgressRail(j);a.setCurrentRail(j)},false);e.addEventListener("timeupdate",function(j){a.setProgressRail(j);
a.setCurrentRail(j)},false);this.loaded=c;this.total=d;this.current=g;this.handle=k},setProgressRail:function(a){var b=a!=undefined?a.target:this.media,c=null;if(b&&b.buffered&&b.buffered.length>0&&b.buffered.end&&b.duration)c=b.buffered.end(0)/b.duration;else if(b&&b.bytesTotal!=undefined&&b.bytesTotal>0&&b.bufferedBytes!=undefined)c=b.bufferedBytes/b.bytesTotal;else if(a&&a.lengthComputable&&a.total!=0)c=a.loaded/a.total;if(c!==null){c=Math.min(1,Math.max(0,c));this.loaded&&this.total&&this.loaded.width(this.total.width()*
=======
...
<<<<<<< HEAD
(function(f){f.extend(mejs.MepDefaults,{muteText:"Mute Toggle",hideVolumeOnTouchDevices:true,audioVolume:"horizontal",videoVolume:"vertical",volumeIconLevels:1,volumeIconLevelClassPrefix:"mejs-volume-level-"});f.extend(MediaElementPlayer.prototype,{buildvolume:function(a,b,c,e){if(!(mejs.MediaFeatures.hasTouch&&this.options.hideVolumeOnTouchDevices)){if(this.options.volumeIconLevels>100)this.options.volumeIconLevels=100;if(this.options.volumeIconLevels<1)this.options.volumeIconLevels=1;var d=this,
g=d.isVideo?d.options.videoVolume:d.options.audioVolume,k=g=="horizontal"?f('<div class="mejs-button mejs-volume-button mejs-mute"><button type="button" aria-controls="'+d.id+'" title="'+d.options.muteText+'"></button></div><div class="mejs-horizontal-volume-slider"><div class="mejs-horizontal-volume-total"></div><div class="mejs-horizontal-volume-current"></div><div class="mejs-horizontal-volume-handle"></div></div>').appendTo(b):f('<div class="mejs-button mejs-volume-button mejs-mute"><button type="button" aria-controls="'+
d.id+'" title="'+d.options.muteText+'"></button><div class="mejs-volume-slider"><div class="mejs-volume-total"></div><div class="mejs-volume-current"></div><div class="mejs-volume-handle"></div></div></div>').appendTo(b),h=d.container.find(".mejs-volume-slider, .mejs-horizontal-volume-slider"),l=d.container.find(".mejs-volume-total, .mejs-horizontal-volume-total"),n=d.container.find(".mejs-volume-current, .mejs-horizontal-volume-current"),p=d.container.find(".mejs-volume-handle, .mejs-horizontal-volume-handle"),
j=function(m,q){if(!h.is(":visible")&&typeof q!="undefined"){h.show();j(m,true);h.hide()}else{m=Math.max(0,m);m=Math.min(m,1);for(var r=d.options.volumeIconLevels,t=1/r,v=d.options.volumeIconLevelClassPrefix,u=0;u<=r;u++)k.removeClass(v+(u+1));if(m>0)for(u=0;u<r;u++)if(t*u<=m&&t*(u+1)>=m){k.addClass(v+(u+1));break}m==0?k.removeClass("mejs-mute").addClass("mejs-unmute"):k.removeClass("mejs-unmute").addClass("mejs-mute");if(g=="vertical"){t=l.height();r=l.position();v=t-t*m;p.css("top",r.top+v-p.height()/
2);n.height(t-v);n.css("top",r.top+v)}else{t=l.width();r=l.position();t=t*m;p.css("left",r.left+t-p.width()/2);n.width(t)}}},s=function(m){var q=null,r=l.offset();if(g=="vertical"){q=l.height();parseInt(l.css("top").replace(/px/,""),10);q=(q-(m.pageY-r.top))/q;if(r.top==0||r.left==0)return}else{q=l.width();q=(m.pageX-r.left)/q}q=Math.max(0,q);q=Math.min(q,1);j(q);q==0?e.setMuted(true):e.setMuted(false);e.setVolume(q)},i=false,o=false;k.hover(function(){h.show();o=true},function(){o=false;!i&&g=="vertical"&&
h.hide()});h.bind("mouseover",function(){o=true}).bind("mousedown",function(m){s(m);f(document).bind("mousemove.vol",function(q){s(q)}).bind("mouseup.vol",function(){i=false;f(document).unbind(".vol");!o&&g=="vertical"&&h.hide()});i=true;return false});k.find("button").click(function(){e.setMuted(!e.muted)});e.addEventListener("volumechange",function(){if(!i)if(e.muted){j(0);k.removeClass("mejs-mute").addClass("mejs-unmute")}else{j(e.volume);k.removeClass("mejs-unmute").addClass("mejs-mute")}},false);
if(d.container.is(":visible")){j(a.options.startVolume);e.pluginType==="native"&&e.setVolume(a.options.startVolume)}}}})})(mejs.$);
...
a.isFullScreen?a.exitFullScreen():a.enterFullScreen()});else{var k=null;if(function(){var i=document.createElement("x"),o=document.documentElement,m=window.getComputedStyle;if(!("pointerEvents"in i.style))return false;i.style.pointerEvents="auto";i.style.pointerEvents="x";o.appendChild(i);m=m&&m(i,"").pointerEvents==="auto";o.removeChild(i);return!!m}()&&!mejs.MediaFeatures.isOpera){var h=false,l=function(){if(h){n.hide();p.hide();j.hide();g.css("pointer-events","");d.controls.css("pointer-events",
"");h=false}},n=f('<div class="mejs-fullscreen-hover" />').appendTo(d.container).mouseover(l),p=f('<div class="mejs-fullscreen-hover"  />').appendTo(d.container).mouseover(l),j=f('<div class="mejs-fullscreen-hover"  />').appendTo(d.container).mouseover(l),s=function(){var i={position:"absolute",top:0,left:0};n.css(i);p.css(i);j.css(i);n.width(d.container.width()).height(d.container.height()-d.controls.height());i=g.offset().left-d.container.offset().left;fullScreenBtnWidth=g.outerWidth(true);p.width(i).height(d.controls.height()).css({top:d.container.height()-
d.controls.height()});j.width(d.container.width()-i-fullScreenBtnWidth).height(d.controls.height()).css({top:d.container.height()-d.controls.height(),left:i+fullScreenBtnWidth})};f(document).resize(function(){s()});g.mouseover(function(){if(!d.isFullScreen){var i=g.offset(),o=a.container.offset();e.positionFullscreenButton(i.left-o.left,i.top-o.top,false);g.css("pointer-events","none");d.controls.css("pointer-events","none");n.show();j.show();p.show();s();h=true}});e.addEventListener("fullscreenchange",
function(){l()})}else g.mouseover(function(){if(k!==null){clearTimeout(k);delete k}var i=g.offset(),o=a.container.offset();e.positionFullscreenButton(i.left-o.left,i.top-o.top,true)}).mouseout(function(){if(k!==null){clearTimeout(k);delete k}k=setTimeout(function(){e.hideFullscreenButton()},1500)})}a.fullscreenBtn=g;f(document).bind("keydown",function(i){if((mejs.MediaFeatures.hasTrueNativeFullScreen&&mejs.MediaFeatures.isFullScreen()||d.isFullScreen)&&i.keyCode==27)a.exitFullScreen()})}},enterFullScreen:function(){var a=
=======
...
<<<<<<< HEAD
this,c=b.tracks[a],e=function(){c.isLoaded=true;b.enableTrackButton(c.srclang,c.label);b.loadNextTrack()};c.isTranslation?mejs.TrackFormatParser.translateTrackText(b.tracks[0].entries,b.tracks[0].srclang,c.srclang,b.options.googleApiKey,function(d){c.entries=d;e()}):f.ajax({url:c.src,success:function(d){c.entries=mejs.TrackFormatParser.parse(d);e();c.kind=="chapters"&&b.media.duration>0&&b.drawChapters(c)},error:function(){b.loadNextTrack()}})},enableTrackButton:function(a,b){if(b==="")b=mejs.language.codes[a]||
...
k;g++)if(d[g].isSeparator)e+='<div class="mejs-contextmenu-separator"></div>';else{var h=d[g].render(c);if(h!=null)e+='<div class="mejs-contextmenu-item" data-itemindex="'+g+'" id="element-'+Math.random()*1E6+'">'+h+"</div>"}c.contextMenu.empty().append(f(e)).css({top:b,left:a}).show();c.contextMenu.find(".mejs-contextmenu-item").each(function(){var l=f(this),n=parseInt(l.data("itemindex"),10),p=c.options.contextMenuItems[n];typeof p.show!="undefined"&&p.show(l,c);l.click(function(){typeof p.click!=
=======
