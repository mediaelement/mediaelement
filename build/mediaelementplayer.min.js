...
false);e.addEventListener("error",function(){d.hide();b.find(".mejs-time-buffering").hide();g.show();g.find("mejs-overlay-error").html("Error loading this resource")},false)}},buildkeyboard:function(a,b,c,e){f(document).keydown(function(d){if(a.hasFocus&&a.options.enableKeyboard)for(var g=0,i=a.options.keyActions.length;g<i;g++)for(var h=a.options.keyActions[g],l=0,n=h.keys.length;l<n;l++)if(d.keyCode==h.keys[l]){d.preventDefault();h.action(a,e,d.keyCode);return false}return true});f(document).click(function(d){if(f(d.target).closest(".mejs-container").length==
...
b.find(".mejs-time-total");c=b.find(".mejs-time-loaded");var g=b.find(".mejs-time-current"),i=b.find(".mejs-time-handle"),h=b.find(".mejs-time-float"),l=b.find(".mejs-time-float-current"),n=function(k){k=k.pageX;var s=d.offset(),j=d.outerWidth(true),o=0;o=0;var m=k-s.left;if(k>s.left&&k<=j+s.left&&e.duration){o=(k-s.left)/j;o=o<=0.02?0:o*e.duration;p&&e.setCurrentTime(o);if(!mejs.MediaFeatures.hasTouch){h.css("left",m);l.html(mejs.Utility.secondsToTimeCode(o));h.show()}}},p=false;d.bind("mousedown",
function(k){if(k.which===1){p=true;n(k);f(document).bind("mousemove.dur",function(s){n(s)}).bind("mouseup.dur",function(){p=false;h.hide();f(document).unbind(".dur")});return false}}).bind("mouseenter",function(){f(document).bind("mousemove.dur",function(k){n(k)});mejs.MediaFeatures.hasTouch||h.show()}).bind("mouseleave",function(){if(!p){f(document).unbind(".dur");h.hide()}});e.addEventListener("progress",function(k){a.setProgressRail(k);a.setCurrentRail(k)},false);e.addEventListener("timeupdate",
function(k){a.setProgressRail(k);a.setCurrentRail(k)},false);this.loaded=c;this.total=d;this.current=g;this.handle=i},setProgressRail:function(a){var b=a!=undefined?a.target:this.media,c=null;if(b&&b.buffered&&b.buffered.length>0&&b.buffered.end&&b.duration)c=b.buffered.end(0)/b.duration;else if(b&&b.bytesTotal!=undefined&&b.bytesTotal>0&&b.bufferedBytes!=undefined)c=b.bufferedBytes/b.bytesTotal;else if(a&&a.lengthComputable&&a.total!=0)c=a.loaded/a.total;if(c!==null){c=Math.min(1,Math.max(0,c));
...
g=d.isVideo?d.options.videoVolume:d.options.audioVolume,i=g=="horizontal"?f('<div class="mejs-button mejs-volume-button mejs-mute"><button type="button" aria-controls="'+d.id+'" title="'+d.options.muteText+'"></button></div><div class="mejs-horizontal-volume-slider"><div class="mejs-horizontal-volume-total"></div><div class="mejs-horizontal-volume-current"></div><div class="mejs-horizontal-volume-handle"></div></div>').appendTo(b):f('<div class="mejs-button mejs-volume-button mejs-mute"><button type="button" aria-controls="'+
...
k=function(m,q){if(!h.is(":visible")&&typeof q=="undefined"){h.show();k(m,true);h.hide()}else{m=Math.max(0,m);m=Math.min(m,1);for(var r=d.options.volumeIconLevels,t=1/r,v=d.options.volumeIconLevelClassPrefix,u=0;u<=r;u++)i.removeClass(v+(u+1));if(m>0)for(u=0;u<r;u++)if(t*u<=m&&t*(u+1)>=m){i.addClass(v+(u+1));break}m==0?i.removeClass("mejs-mute").addClass("mejs-unmute"):i.removeClass("mejs-unmute").addClass("mejs-mute");if(g=="vertical"){t=l.height();r=l.position();v=t-t*m;p.css("top",Math.round(r.top+
v-p.height()/2));n.height(t-v);n.css("top",r.top+v)}else{t=l.width();r=l.position();t=t*m;p.css("left",Math.round(r.left+t-p.width()/2));n.width(Math.round(t))}}},s=function(m){var q=null,r=l.offset();if(g=="vertical"){q=l.height();parseInt(l.css("top").replace(/px/,""),10);q=(q-(m.pageY-r.top))/q;if(r.top==0||r.left==0)return}else{q=l.width();q=(m.pageX-r.left)/q}q=Math.max(0,q);q=Math.min(q,1);k(q);q==0?e.setMuted(true):e.setMuted(false);e.setVolume(q)},j=false,o=false;i.hover(function(){h.show();
o=true},function(){o=false;!j&&g=="vertical"&&h.hide()});h.bind("mouseover",function(){o=true}).bind("mousedown",function(m){s(m);f(document).bind("mousemove.vol",function(q){s(q)}).bind("mouseup.vol",function(){j=false;f(document).unbind(".vol");!o&&g=="vertical"&&h.hide()});j=true;return false});i.find("button").click(function(){e.setMuted(!e.muted)});e.addEventListener("volumechange",function(){if(!j)if(e.muted){k(0);i.removeClass("mejs-mute").addClass("mejs-unmute")}else{k(e.volume);i.removeClass("mejs-unmute").addClass("mejs-mute")}},
false);if(d.container.is(":visible")){k(a.options.startVolume);e.pluginType==="native"&&e.setVolume(a.options.startVolume)}}}})})(mejs.$);
...
a.isFullScreen?a.exitFullScreen():a.enterFullScreen()});else{var i=null;if(function(){var j=document.createElement("x"),o=document.documentElement,m=window.getComputedStyle;if(!("pointerEvents"in j.style))return false;j.style.pointerEvents="auto";j.style.pointerEvents="x";o.appendChild(j);m=m&&m(j,"").pointerEvents==="auto";o.removeChild(j);return!!m}()&&!mejs.MediaFeatures.isOpera){var h=false,l=function(){if(h){n.hide();p.hide();k.hide();g.css("pointer-events","");d.controls.css("pointer-events",
"");h=false}},n=f('<div class="mejs-fullscreen-hover" />').appendTo(d.container).mouseover(l),p=f('<div class="mejs-fullscreen-hover"  />').appendTo(d.container).mouseover(l),k=f('<div class="mejs-fullscreen-hover"  />').appendTo(d.container).mouseover(l),s=function(){var j={position:"absolute",top:0,left:0};n.css(j);p.css(j);k.css(j);n.width(d.container.width()).height(d.container.height()-d.controls.height());j=g.offset().left-d.container.offset().left;fullScreenBtnWidth=g.outerWidth(true);p.width(j).height(d.controls.height()).css({top:d.container.height()-
d.controls.height()});k.width(d.container.width()-j-fullScreenBtnWidth).height(d.controls.height()).css({top:d.container.height()-d.controls.height(),left:j+fullScreenBtnWidth})};f(document).resize(function(){s()});g.mouseover(function(){if(!d.isFullScreen){var j=g.offset(),o=a.container.offset();e.positionFullscreenButton(j.left-o.left,j.top-o.top,false);g.css("pointer-events","none");d.controls.css("pointer-events","none");n.show();k.show();p.show();s();h=true}});e.addEventListener("fullscreenchange",
function(){l()})}else g.mouseover(function(){if(i!==null){clearTimeout(i);delete i}var j=g.offset(),o=a.container.offset();e.positionFullscreenButton(j.left-o.left,j.top-o.top,true)}).mouseout(function(){if(i!==null){clearTimeout(i);delete i}i=setTimeout(function(){e.hideFullscreenButton()},1500)})}a.fullscreenBtn=g;f(document).bind("keydown",function(j){if((mejs.MediaFeatures.hasTrueNativeFullScreen&&mejs.MediaFeatures.isFullScreen()||d.isFullScreen)&&j.keyCode==27)a.exitFullScreen()})}},enterFullScreen:function(){var a=
